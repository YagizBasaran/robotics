...
# Adjust the bounding box color to blue if desired
...
f_bbox.setColor([0, 0, 1, 0.2])  # Changed to blue
C.view()
...
for cam_frame in camera_frames:
    ...
    if pcl_cropped.size > 0:
        ...
        f_crop.setPointCloud(points=pcl_cropped, colors=[0.0, 0.0, 1.0])  # Changed to blue
        C.view()
...
f_pcl.setPointCloud(points=merged_pcl, colors=[0.0, 0.0, 1.0])  # Changed to blue
C.view()
...
# Manually specify colors for clusters
...
# Visualization with adjusted styles
o3d.visualization.draw_geometries(
    [pcd],
    point_show_normal=True,
    window_name="Segmented Point Cloud with Normals",
    width=800,
    height=600
)

o3d.io.write_point_cloud("segmented_point_cloud_with_normals.ply", pcd)
print("Segmented point cloud with normals saved to 'segmented_point_cloud_with_normals.ply'.")
